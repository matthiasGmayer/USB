...
public class Client : MonoBehaviour
{
    ...
    public void Connect()
    {
		...
		//gets the Hostname from the main textfield
        if ("".Equals(hostName))
        {
			//Not Implemented
    ...
    void Update()
    {
        if (changed)
        {
			//if a picture was send, load it into the texture that is displayed
    ...
    public void StartWiFiClient()
    {
        try
        {
            ...
            try
            {
				...
				//new Threads, to not interrupt the UI
                ...
                void send()
                {
                    while (!abort)
                    {
						//send the slider values as Ascii Strings of Floats
                ...
                void receive()
                {
					...
					//All Received Bytes get put in a list after on another
					//then the length gets read, and the appropriate bytes get put in a list
					//all used up bytes get removed from the worklist
					//this is list is then processed by Unity to display the picture
                    List<byte> byteList = new List<byte>();
	...
	//sets the Slider value, this function is called by the Sliders through scripting
